...
const ev = new Event('fd.c2s.login.request', ({ socket, args, loginList }) => {
  const sid = args[0];
  if (!sid) return;
  ...
  socket.emit('fd.c2s.login.request_accepted', '/assets/tools/login.html', settings.LoginMessage, settings.YourName);
