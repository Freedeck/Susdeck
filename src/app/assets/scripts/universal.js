...
const universal = {
  ...
  sendToast: (message) => {
    ...
    universal.save('notification_log', universal.load('notification_log') + `,${btoa(btoa(atob(btoa(JSON.stringify({ time: new Date().toTimeString(), page: window.location.pathname, message })))))}`);
  },
  validSession: () => {
