...
const sc = {
	...
	checkStyle: async () => {
		try {
				if (!await fs.access(styleLocation, (e) => {
						if (e) {
								debug.log(`Error checking style configuration: ${e}`, "Style / Migration");
						}
				})) {
						const def = JSON.stringify(sc.styleSettings);
						await fs.writeFile(styleLocation, def, (e) => {
								if (e) {
										debug.log(`Error creating style configuration: ${e}`, "Style / Migration");
								}
						});
						debug.log("Created style configuration file.", "Style / Migration");
				} else {
						const data = JSON.parse(await fs.readFile(styleLocation, (e) => {
								if (e) {
										debug.log(`Error reading style configuration: ${e}`, "Style / Migration");
								}
						}));
						...
						await fs.writeFile(styleLocation, JSON.stringify(data), (e) => {
								if (e) {
										debug.log(`Error saving style configuration: ${e}`, "Style / Migration");
								} else {
